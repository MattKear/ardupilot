...
function update() -- this is the loop which periodically runs
  ...
  if mission_state == 2 then --MISSION_COMPLETE
    gcs:send_text(0, "LUA: Mission Complete")
    return update, 1000 -- reschedules the loop
  elseif mission_state == 0 then --MISSION_STOPPED
  ...
  if mission_index ~= last_mission_index then
    ...
    -- print the current and prevous nav commands
    ...
    if mission_index == mission_length then
      ...
      local jump_to = math.random(mission_length - 1)  -- no point jump to the end so - 1
      local set_succeed = mission:set_current_cmd(jump_to)
      if set_succeed then

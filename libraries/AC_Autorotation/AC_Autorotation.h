...
public:
    ...
    //Constructor
    AC_Autorotation(AP_MotorsHeli*& motors, AC_AttitudeControl*& att_crtl, AC_PosControl*& pos_ctrl, AP_Int16& land_speed_cm);
    ...
    void log_write_autorotation(uint8_t phase) const;
...
private:
    ...
    // Taking the 3rd instance of SurfaceDistance to not conflict with the first two already in copter, we do this so that we do not have
    // to play with the enabled state in copter's surface tracking avoiding getting out of sync with the RC switch position.
    AP_SurfaceDistance _ground_surface{ROTATION_PITCH_270, 2U}; 
#endif
    ...
    bool get_mean_norm_headspeed(float& norm_rpm) const; // Get mean of both heads if dual is enabled
    ...
    const float COLLECTIVE_CUTOFF_FREQ_HZ = 2.0; // (Hz)
};

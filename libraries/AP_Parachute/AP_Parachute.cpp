...
const AP_Param::GroupInfo AP_Parachute::var_info[] = {
    ...
    // @User: Standard
    AP_GROUPINFO_FLAGS("ENABLED", 0, AP_Parachute, _enabled, 1, AP_PARAM_FLAG_ENABLE),
...
void AP_Parachute::update()
{
    ...
    if (!_release_setup || !_release_in_progress) {
        if (!_release_setup) {
            ...
            if ((_options.get() & uint32_t(OPTIONS::DONT_DISARM)) == 0) {
                // force disarm
    ...
    if (_release_in_progress && time_diff <= delay_ms + AP_PARACHUTE_RELEASE_DURATION_MS) {
        // send the send_chute_msg every 100ms
        if ((now - _last_chute_msg_ms) > 100) {
            _last_chute_msg_ms = now;

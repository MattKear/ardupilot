...
void AP_RPM_Pin::update(void)
{
    ...
    if (irq_state[state.instance].dt_count > 0) {
        ...
        const float scaling = ap_rpm._params[state.instance]._scaling;
        float maximum = ap_rpm._params[state.instance]._maximum;
        float minimum = ap_rpm._params[state.instance]._minimum;
        float quality = 0;

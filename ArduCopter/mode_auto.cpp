...
{
    // do not add next wp if current command has a delay (stored in p1 for normal and spline waypoints). Returning early means the vehicle will stop at the destination (i.e. not a fast wp)
    ...
    case MAV_CMD_NAV_ARC_WAYPOINT: {
        const Location dest_loc = loc_from_cmd(current_cmd, default_loc);
        const Location next_dest_loc = loc_from_cmd(next_cmd, dest_loc);
        const float arc_angle = radians(next_cmd.get_arc_angle_deg());

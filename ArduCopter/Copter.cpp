...
const AP_Scheduler::Task Copter::scheduler_tasks[] = {
    ...
    SCHED_TASK(fourhundred_hz_logging,400,    65,  75),
#endif
    SCHED_TASK_CLASS(GCS,                  (GCS*)&copter._gcs,          update_receive, 400, 450, 76),
    SCHED_TASK(ccdl_failover_check,   25,    100,  77),
    ...
    SCHED_TASK_CLASS(GCS,                  (GCS*)&copter._gcs,          update_send,    400, 650, 105),
...
void Copter::ccdl_failover_check()
{
    if (g2.ccdl_timeout_enabled) {
        ...
        const bool use_longer_timeout = copter.ap.in_arming_delay || copter.arming.in_arm_motors || AP::logger().in_log_download();
        const uint32_t current_failover_timeout_ms = use_longer_timeout ? GCS_MAVLINK::CCDL_FAILOVER_TIMEOUT_ARMING_MS  : GCS_MAVLINK::CCDL_FAILOVER_TIMEOUT_MS;
        const uint32_t current_failover_backup_timeout_ms = use_longer_timeout ? GCS_MAVLINK::CCDL_FAILOVER_BACKUP_TIMEOUT_ARMING_MS : GCS_MAVLINK::CCDL_FAILOVER_BACKUP_TIMEOUT_MS;
        ...
        if (ccdl_timeout[ccdl0_target].timeout_ccdl && ccdl_timeout[ccdl1_target].timeout_ccdl) {
            ...
            for (uint8_t i = 0; i < 2; i++) {
                const auto target = GCS_MAVLINK::ccdl_routing_tables[my_id].ccdl[i].primary_route_sysid_target - 1;
